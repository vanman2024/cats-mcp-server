...
async def test_every_returned_job_names_the_field_that_matched():
    ...
    by_id = {row["job_id"]: row for row in result.data["jobs"]}
...
async def test_a_job_matched_by_company_says_company():
    ...
    row = next(r for r in result.data["jobs"] if r["job_id"] == 2)
...
async def test_a_job_matched_by_a_custom_field_names_that_field():
    ...
    row = next(r for r in result.data["jobs"] if r["job_id"] == 3)
...
async def test_a_description_match_returns_a_window_not_the_whole_document():
    ...
    entry = result.data["jobs"][0]["matched_fields"][0]
...
async def test_an_unknown_id_is_reported_rather_than_raised():
    ...
    assert data["jobs"] == []
    assert data["count"] == 0
    assert data["errors"], "a missing id must be reported, not silently empty"
...
async def test_a_status_seed_uses_one_exact_filter_per_value():
    ...
    assert result.data["seeds_used"] == [
        {"field": "status_id", "value": 11},
        {"field": "status_id", "value": 12},
    ]
    assert result.data["count"] == 1, "the same job under two seeds is one row"
...
async def test_the_budget_is_respected_and_what_it_missed_is_reported():
    ...
    data = result.data
    assert data["requests_used"] == 4, "one sweep page plus three per-job reads"
    ...
    assert "custom_fields" in data["errors"]
