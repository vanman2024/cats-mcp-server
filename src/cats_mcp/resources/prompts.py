...
def register(mcp: Any) -> int:
    ...
    def search_by_location(towns: str = "", within_km: str = "") -> str:
        ...
        return (
            ...
            "commute from. Establish the list of towns in scope first.\n"
            "\n"
            "   The API lists a 'geo_distance' filter for radius matching, but the "
            "field it applies to is unconfirmed - it returns 400 on both 'city' and "
            "'postal_code'. Use the explicit town list, which is verified.\n"
            "\n"
